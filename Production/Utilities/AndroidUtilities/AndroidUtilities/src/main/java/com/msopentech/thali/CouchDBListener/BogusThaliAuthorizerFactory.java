...
public class BogusThaliAuthorizerFactory implements AuthorizerFactory {
    ...
    public Authorizer getAuthorizer(ReplicatorArguments replicatorArguments) throws CouchbaseLiteException {
        Map<String, Object> authParms = replicatorArguments.getPush() ? replicatorArguments.getTargetAuth() : replicatorArguments.getSourceAuth();
        ...
        try {
            ...
            if (childOfAuthParams != null && childOfAuthParams.size() != 0) {
                throw new CouchbaseLiteException(thaliFieldName + " must be an empty object", new Status(Status.BAD_REQUEST));
            }
        } catch (ClassCastException e) {
            throw new CouchbaseLiteException(thaliFieldName + " must be a JSON object", new Status(Status.BAD_REQUEST));
        }

        String remoteHttpKeyURL = replicatorArguments.getPush() ? replicatorArguments.getTarget() : replicatorArguments.getSource();
        ...
        try {
            ...
            throw new CouchbaseLiteException("remote URL value " + remoteHttpKeyURL + " is not a valid HttpKeyURL.", new Status(Status.BAD_REQUEST));
