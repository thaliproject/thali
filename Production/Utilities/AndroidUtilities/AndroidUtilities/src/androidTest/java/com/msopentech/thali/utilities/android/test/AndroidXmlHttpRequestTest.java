/*
Copyright (c) Microsoft Open Technologies, Inc.
All Rights Reserved
Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the
License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0

THIS CODE IS PROVIDED ON AN *AS IS* BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, EITHER EXPRESS OR IMPLIED,
INCLUDING WITHOUT LIMITATION ANY IMPLIED WARRANTIES OR CONDITIONS OF TITLE, FITNESS FOR A PARTICULAR PURPOSE,
MERCHANTABLITY OR NON-INFRINGEMENT.

See the Apache 2 License for the specific language governing permissions and limitations under the License.
*/

package com.msopentech.thali.utilities.android.test;

import android.os.Build;
import android.test.ActivityInstrumentationTestCase2;
import com.msopentech.thali.utilities.android.AndroidXmlHttpRequestTestActivity;
import com.msopentech.thali.utilities.xmlhttprequestbridge.BridgeTestManager;

public class AndroidXmlHttpRequestTest extends ActivityInstrumentationTestCase2<AndroidXmlHttpRequestTestActivity> {
    protected BridgeTestManager bridgeTestManager = null;

    public AndroidXmlHttpRequestTest() {
        super(AndroidXmlHttpRequestTestActivity.class);
    }

    @Override
    protected void setUp() {
        bridgeTestManager = new BridgeTestManager();
    }

    public void testXmlHttpRequest() throws InterruptedException {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            throw new RuntimeException("This test only runs in KitKat or higher!");
        }
        getActivity().runTest(bridgeTestManager, getInstrumentation().getContext());
        assertTrue(bridgeTestManager.testResult());
    }
}
