...
We are using [codereviewhub.com](codereviewhub.com) to make code reviewing PRs on GitHub a tiny bit less painful. Please click on that link and watch the 95 second video. It really does explain everything. The general rule is that the only person who can :+1: a comment to close it is the person who created. So as the person whose code is being reviewed please just respond to the comment when you have fixed it and then the person who created the comment will get an email and can then see if they agree and if so :+1:.
